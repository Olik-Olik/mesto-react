...
{/*Показывайте полноразмерную картинку при клике*/}
 <ImagePopup
  ...
  <section className="popup popup_type_edit">
    ...
    <div className="popup__container">
      <form action="#" aria-label='получения инфо и передачи данных' className="popup__form" id="popup-mega-id"  method="GET" name="resaveProfile" noValidate>
        <h2 className="popup__page">Редактировать профиль</h2>
        <label className="popup__label">
          <input className="popup__field" id="popup-field-name"  maxLength="40" minLength="2" name="inputForm_name" placeholder="Ваше имя" required type="text"  value='Жак-Ив Кусто'/>
          <span className="popup__input-error" id="popup-field-name-error"/>
        </label>
        <label className="popup__label">
          <input className="popup__field" id="popup-field-job" maxLength="200" minLength="2" name="inputForm_job"   placeholder="Род занятия" required type="text" value='Исследователь океана'/>
  ...
  <section className="popup popup_country">
    <button aria-label='Закрыть всплывающее окошко' className="popup__close-button" type="button" />
    <div className="popup__container">
      <form action="#" aria-label='получения инфо и передачи данных в адресной строке' className="popup__form" id="popup-input-mega-id" method="GET" name="resaveCountry" noValidate>
        <label className="popup__label">
          <h2 className="popup__page">Редактировать Новое место</h2>
          <input className="popup__field" id="popup-field-card-name" maxLength="30" minLength="2" name="popup-input-place" placeholder="Название" required type="text" value=''/>
          <span className="popup__input-error" id="popup-field-card-name-error" />
        </label>
        <label className="popup__label">
          <input className="popup__field" id="popup-field-card-img" name="popup-input-img" placeholder="Ссылка на картинку"  required type="url" value=''/>
  ...
  <section className="popup popup_type_image">
    <div className="popup__combine-image">
      {/*<div style="alignment: right">*/}
        <div>
        <button aria-label='Закрыть всплывающее окошко' className="popup__close-button popup__close-button-no-rel" type="button"/>
      </div>
  ...
  <section className="popup popup_delete-confirm">
    ...
    <div className="popup__container-delete-confirm">
      <form action="#" aria-label='Вы уверены, что хотите удалить карточку?' className="popup__form" id="popup-delete-card" name="deleteConfirmCard" noValidate>
        <label className="popup__label"><h2 className="popup__page">Вы уверены?</h2></label>
        <button aria-label='Кнопка уверенности в закрытии' className="popup__save" type="submit">Да</button>
      </form>
    </div>
